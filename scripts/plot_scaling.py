...
def plot_mem():
    ...
    f1_size, f2_size, f1_peak, f2_peak = measure_mem(range(2, 100), implicit.simulate, evaltrap_kwargs, explicit.simulate, evalrk4_kwargs)
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.title("Memory improvement using RK4")
    return


def plot_speed():
    evaltrap_kwargs = {'x': None, 'p': None, 'u': None, 't': 40, 'delta_t': 1e-2, 'f_step': implicit.get_trapezoid_f}
    evalrk4_kwargs = {'x': None, 'p': None, 'u': None, 't': 40, 'delta_t': 1e-2, 'f_step': explicit.rk4}
